...
Send with `herdr agent prompt <name> "<text>"`. Use `--wait --timeout` for short pm/po turns; send dev and qa turns plain (no `--wait`) — they run for many minutes, and the watcher (§Watcher) is what brings the orchestrator back.
...
The orchestrator's wake channel. A prompt sent without `--wait` is fire-and-forget: the orchestrator session is idle by the time the role finishes, so nothing advances — unless a watcher is armed. Arm one with the Bash tool's `run_in_background` every time you dispatch a dev or qa turn (or enter a waiting stage and no watcher is live):

```bash
herdr agent wait <role> --until working --until blocked --timeout 60000 \
  && herdr agent wait <role> --until idle --until done --until blocked
```

When the command exits, the harness re-invokes this session — that wake is the "Watcher wake (auto-drive)" turn in SKILL.md.

Why two stages: right after `agent prompt` the role may still report its previous `idle`, which would end the wait instantly. The first stage confirms the turn actually started (or catches an instant `blocked` question); the second waits for the end with no timeout. Each `--until` takes exactly one state — repeat the flag, never comma-join them. Any exit wakes the orchestrator — including the 60 s timeout (the turn never started; read the pane and recover) or a hard error (the agent is likely gone; §Recovery).

Rules:

- One watcher at a time — the pipeline's stages are sequential.
- Watchers are ephemeral: they die with the orchestrator session. `/team go` re-arms a missing watcher whenever the stage says a role should be working, so after a session restart one `/team go` restores auto-drive.
- A wake performs at most one stage advance, then re-arms. Terminal or user-decision stages (done, stuck, blocked, ESCALATE) stop auto-drive: report and do not re-arm.
- Gates (pm-spec, dev-ticket, qa-kickoff) stop only while they are in team.md's `Gates:` set. `/team go <gates>` rewrites that set — `all` (default), a subset like `dev-ticket`, or `none` for hands-off.
- `/team hold` cancels the watcher: stop the background task with TaskStop (or the equivalent background-task kill), report, and advance nothing until `/team go`. In-flight role turns are not recalled — only orchestration freezes.
- Before re-arming, check the role is not already at the target state — a watcher on an `idle` role fires immediately, which is a harmless no-op wake, but avoid arming one when no turn is actually due.
...
- Gates: all              # all | none | subset of pm-spec, dev-ticket, qa-kickoff — via /team go
...
- **Dead agent** (pane closed, name gone from `herdr agent list`; also what a watcher exiting with a hard error usually means): split a replacement pane with the same geometry, `agent start` the same name, re-send the current stage's prompt and re-arm the watcher — the files carry all state.
- **Stale in-progress ticket with idle dev**: read the pane tail. If the turn ended without the Status flip, prompt dev to finish the ticket; if the work is lost, set the ticket back to ready-for-agent and re-prompt. Either way re-arm the watcher.
...
- **Watcher wake with nothing due** (role idle, files say waiting): the wait fired on a stale state or a stray no-op. Derive the stage, re-arm if a turn is genuinely still running, otherwise report and stop. Never treat a wake as license to re-prompt a role out of turn.
