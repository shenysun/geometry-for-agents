---
name: team
description: "Run the four-role Herdr team (pm, po, dev, qa panes) that ships one feature of this repo. In the main session the user runs /team as the orchestrator (up, status, go [all|none|gates], hold, down); in a role pane the orchestrator prompts /team role: pm|po|dev|qa. Requires HERDR_ENV=1."
...
You are not a fifth worker: you create the layout, route turns, and surface escalations to the user. Long turns are not fire-and-forget: after dispatching one you arm a **watcher** (a background `herdr agent wait`) that wakes this session when the role lands, and the wake advances the pipeline (§Watcher wake). Advancement is paced by the run's **active gates** — the stages that wait for `/team go` instead of acting. All three gates are active by default; `/team go <gates>` narrows the set. For exact herdr command sequences, prompt texts, and the team.md format, load [references/orchestration.md](references/orchestration.md).
...
4. Tell the user: PM will interview via PO, then run `/team go` — the watcher wakes the orchestrator on every role landing; by default spec, every ticket, and qa each wait for `/team go` (`/team go dev-ticket` or `/team go none` narrows it). Mention the cost: this runs four parallel Claude sessions.
...
1. The optional argument rewrites the `Gates:` line in team.md for the rest of the run: `all` (the default for a fresh team) = pm-spec, dev-ticket, qa-kickoff; `none` = hands-off; or a comma list naming a subset, e.g. `dev-ticket` or `pm-spec,qa-kickoff`. Without an argument the current set stands.
2. Derive the stage (§Stage) and perform only that stage's action. Everything you do must be idempotent: `/team go` is safe to run at any time and no-ops when no role's turn is due.
3. If the stage is an **active gate** (in the `Gates:` set): do NOT act. Report what is ready, name the exact action the next `/team go` will take, and stop without arming. Inactive gates and non-gate stages act normally — that is how a subset intercepts only what the user chose.
4. Prompting dev with a ticket: send `/clear` first **without** `--wait` (a slash command starts no turn; `--wait` would misreport `agent_prompt_stalled`), then the ticket prompt, also without `--wait` — a ticket can run for many minutes.
5. Arm the watcher whenever you dispatch a long turn (dev ticket, qa kickoff, qa regression) or enter a waiting stage (dev-working, qa-running) — exact command in orchestration.md §Watcher. The watcher wakes this session when the role lands, so the pipeline keeps moving.
6. Report what you did, and what happens next: which role is running, that the watcher will wake you, and the stop conditions (an active gate, blocked question, ESCALATE, done).

### hold

Kill switch for auto-drive: stop the background watcher task and stop advancing. Role panes are untouched — an in-flight dev/qa turn keeps running; if the user wants that stopped too, they focus the pane and interrupt manually. Report the stage and live role states. `/team go` re-arms and resumes.

### Watcher wake (auto-drive)

When a background `herdr agent wait` exits, this session is re-invoked: the role either finished its turn (`idle`/`done`), hit a question (`blocked`), or the wait itself failed. One wake, one advance:

1. `herdr agent get <role>`. `blocked` → the blocked-role handling below, then stop (no re-arm). Missing name → orchestration.md §Recovery (dead agent).
2. Otherwise run the `go` logic exactly once: derive the stage from files, hold if it is an active gate (report and stop), otherwise perform that stage's action and arm the next watcher if the new stage waits on a role. Never loop through multiple stages inside one wake — each advance re-derives from files first.
3. Report in two or three lines what advanced and what runs now. Terminal stages (`done`, `stuck`), active gates, and anything awaiting the user stop auto-drive: do not re-arm.
...
| a ticket is in-progress | dev-working | arm the watcher on dev if not yet armed; if dev is idle without the Status flip, read the pane and recover (orchestration.md §Recovery) |
| all tickets done, no acceptance.md | qa-kickoff | prompt qa: kickoff, then arm the watcher on qa |
| acceptance.md Status: in-progress | qa-running | arm the watcher on qa if not yet armed; surface anything blocked |
...
Stages **pm-spec**, **dev-ticket**, and **qa-kickoff** are the gates — each publishes a document or spends a full dev/qa session. Only the gates in team.md's `Gates:` set actually wait for `/team go`; the rest act automatically.
